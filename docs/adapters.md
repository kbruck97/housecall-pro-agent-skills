...
## Optional local connection provider

The `hcp-connections` skill now provides local interactive login, encrypted per-company session storage and fixed identity verification. It does not supply a wildcard HTTP transport or business-operation authorization. A downstream adapter must consume a verified in-process connection, retain exact company binding, enforce its own reviewed operation allowlist and keep all credentials out of model-visible output. Follow the [session workflow](internal-session-workflow.md); a standalone skill must load `hcp-connections` separately when it needs this runtime.
